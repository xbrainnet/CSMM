...
def train_and_evaluate(model, subjectIndex, source_data, source_label, target_data, target_label, domain_size):
    ...
    for epoch in range(n_epoch):
        ...
        torch.save(model, 'sdann_models_save_IV/1/model_epoch_current.pth')
        my_net = torch.load('sdann_models_save_IV/1/model_epoch_current.pth')
        my_net = my_net.eval()
        ...
        if accu_t > best_accu_t:
            best_accu_s = accu_s
            best_accu_t = accu_t
            torch.save(my_net, 'sdann_models_save_IV/1/model_epoch_best{}.pth'.format(subjectIndex))
    print('============ Summary ============= \n')
    print('Accuracy of the %s dataset: %f' % ('11 source domain', best_accu_s))
    print('Accuracy of the %s dataset: %f' % ('target', best_accu_t))
    print('Corresponding model was save in sdann_models_save_IV/1/model_epoch_best{}.pth'.format(subjectIndex))
...
if __name__ == '__main__':
    ...
    for subjectIndex in range(len(filenameList)):
        ...
        model_name = 'sdann_models_save_IV/1/model_epoch_best' + str(subjectIndex) + '.pth'
    ...
    df = DataFrame(data)
    df.to_excel('SEED_IV_result/1/result1_2.xlsx')
